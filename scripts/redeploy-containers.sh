...
(docker stop server && docker rm server && false) || (docker stop service_registry && docker rm service_registry && false) || (docker stop client && docker rm client && false) || true
